...
- **Post-MVP:** A user can only read/write their own diagrams. Share links grant read-only access to anyone with the token. Blueprints are readable by all authenticated users. Blueprint management is via hard-coded changes to the project.
